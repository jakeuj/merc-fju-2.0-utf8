...
- `player/` 採 `player/<letter>/<name>/data` 結構，runtime 會以角色名稱首字母的**小寫**版本決定 bucket（例如 `Alice -> player/a/alice/data`）。伺服器在存檔時會自動補建缺少的 bucket 與玩家目錄；`scripts/clean-runtime.sh` 仍會預先整理 `player/`，方便重置測試/打包環境。
