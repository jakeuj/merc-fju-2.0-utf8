...
## 快速開始（6 步驟）
1. **規劃**：決定區域 slug（`area/<slug>`）、VNUM 區段、`Serial`、`Capital`、故事描述。先確認 `area/` 內沒有同名目錄，也不與既用 VNUM 衝突；預設不要只留 `99` 個號碼，至少先以「一區保留一個千位段」的思路規劃，例如 `10000-10999` 整段保留給同一正式區，再在區內自行切 `mob/obj/roo` 的子段。
...
6. **驗證**：先執行 `python3 scripts/check-data.py`（整體 UTF-8 與基本標記），再額外做一次全區 VNUM 重複掃描（至少檢查 `mob/obj/roo` 檔名是否撞號），視需要用搜尋工具再次檢查出入口、裝備、商店、reset 與玩家提示是否仍引用舊區，最後啟動 `./startup` 或 `./start-merc.sh start`，確認 `debug/bugs`、`debug/error` 與 `log/manual-start-*.log` 沒有 `Load_room`、`load_mobiles`、重複房號或引用不存在的錯誤。
...
- 區域開發的起點是參考既有區域檔案。現況已切成正式區群 `academy`、`changan`、`xuchang`、`drillground`，且採一區一千位段保留策略；新工作優先比對這些正式區與 `limbo`，不要再依賴已移除的釋出區。
...
## 規劃建議
- **VNUM 與 Serial**：保持連號方便查詢。主城或會長期擴充的正式區，預設保留一整個 `1000` 號區段。現況正式世界建議視為 `10000-10999 = changan`、`11000-11999 = xuchang`、`12000-12999 = academy`、`13000-13999 = drillground`。小型一次性區域才考慮在大段裡再細切子段。
- **區內切段**：即使整區保留 `1000` 號，也不要混著亂用；建議在該千位段內再自行分配，例如 `+000-099` 給核心房間、`+100-299` 給一般房間、`+300-499` 給 mob、`+500-699` 給 obj、`+700-799` 給商店/服務 NPC、`+800-899` 給保留擴充、`+900-999` 給特殊事件或跨系統接口。
- **載入清單註解**：建議在 `area/directory.lst` 每個正式區前面加註保留號碼範圍註解，例如 `* changan: 10000-10999`、`* academy: 12000-12999`。這不能取代實際掃描，但能大幅降低手動搬移時撞號的機率。
- **命名**：檔名與 VNUM 相同（`10001.mob`、`10001.roo`）；若採單一 `res` 檔，使用 `<slug>.res`。
- **避免抓太緊**：不要因為第一版只做了 20 個房間、6 個 NPC，就只保留 30 個號碼。只要是預期會持續成長、會放商店、交通、皇宮、任務或戰鬥層的區域，起手就保留寬鬆範圍。
